...
export function createProduct({commit}, product) {
    if (product.tour_cover instanceof File) {
        ...
        form.append('video_url', product.video_url ?? '');
...
/* Galleries */
export function getGalleries({commit}, {url = null, perPage = 10} = {}) {
    url = url || '/galleries';
    return axiosClient.get(url, {
        params: {perPage}
    }).then(({data}) => {
...
export function getGallery({commit}, id) {
    return axiosClient.get(`/galleries/${id}`)
}

export function createGallery({commit}, gallery) {
    if (gallery.image instanceof File) {
        const form = new FormData();
        form.append('alternative', gallery.alternative);
        form.append('image', gallery.image);
        gallery = form
    }
    return axiosClient.post('/galleries', gallery)
}

export function updateGallery({commit}, gallery) {
    const id = gallery.id;
    if (gallery.image instanceof File) {
        const form = new FormData();
        form.append('alternative', gallery.alternative);
    ...
    return axiosClient.post(`/galleries/${id}`, gallery);
}

export function deleteGallery({commit}, id) {
